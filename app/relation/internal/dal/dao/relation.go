...
func (dao *RelationDao) GetFollowByUserId(userId1 int64, userId2 int64) bool {
	relationship := model.Relation{}
	if userId1 == userId2 || userId1 == 0 {
		return false
	}
	if err := _db.Model(&model.Relation{}).Where("host_id=? And guest_id=?", userId1, userId2).First(&relationship).Error; err == gorm.ErrRecordNotFound {
		return false
	}
	return true
}
...
func (dao *RelationDao) CreateFollowing(Host_id int64, Guest_id int64) error {
	...
	//2.新建following
	if err := _db.Model(&model.Relation{}).Create(&newFollowing).Error; err != nil {
		return err
	}
	return nil
}
...
func (dao *RelationDao) DeleteFollowing(Host_id int64, Guest_id int64) error {
	//1.Following数据模型准备
	...
	//2.删除following
	if err := _db.Model(&model.Relation{}).Where("host_id=? AND guest_id=?", Host_id, Guest_id).Delete(&deleteFollowing).Error; err != nil {
		return err
	}
...
func (dao *RelationDao) IsFollower(Host_id int64, Guest_id int64) (err error) {
	var relationExist = &model.Relation{}
	//判断关注是否存在
	err = _db.Model(&model.Relation{}).
